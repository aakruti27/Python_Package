...
class Binomial(Distribution):
    ...
    def calculate_stdev(self):
        ...
        dev= (self.n * self.p *(1- self.p))**0.5
        self.stdev=dev
        return self.stdev
    ...
    def replace_stats_with_data(self):
        ...
        self.p= float(sum(self.data)/len(sekf.data))
        self.mean=self.calculate_mean()
        self.stdev=self.caluclate_stdev()
    ...
    def pdf(self, k):
        ...
        pdfval=(math.factorial(self.n)/(math.factorial(k)*math.factorial(n-k)))*(p**k)*((1-p)**(n-k))
        return pdfval        
